...
export const merchantController = {
  ...
  async getTransactions(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const where: any = { merchantId, createdAt: { gte: since } };
      if (search) {
        where.OR = [
      ...
      const [transactions, total] = await Promise.all([
        prisma.transaction.findMany({
          ...
          select: {
            ...
            status: true, method: true, customerName: true, customerEmail: true,
            customerPhone: true, country: true, flag: true, isCredit: true,
