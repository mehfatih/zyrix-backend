...
export const getCLVOverview = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const avgCLV    = Math.round((agg._avg.totalSpent    ?? 0) * 100) / 100;
    const avgAOV    = Math.round((agg._avg.avgOrderValue ?? 0) * 100) / 100;
    const avgOrders = Math.round((agg._avg.totalOrders   ?? 0) * 10)  / 10;
    const totalRev  = Math.round((agg._sum.totalSpent    ?? 0) * 100) / 100;
...
// ─── GET /api/customer-clv/segments ─────────────────────────────────────────
export const getSegments = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const merchantId = req.merchant!.id;

    const [champions, loyal, atRisk, lost, new_customers] = await Promise.all([
      // Champions: high spend + recent + many orders
      prisma.customer.count({
        where: { merchantId, totalOrders: { gte: 5 }, totalSpent: { gte: 1000 },
          lastSeenAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } },
      }),
      // Loyal: decent orders + spending
      prisma.customer.count({
        where: { merchantId, totalOrders: { gte: 3 }, totalSpent: { gte: 300 } },
      }),
      // At Risk: haven't bought in 60-90 days
      prisma.customer.count({
        where: {
          merchantId,
          lastSeenAt: {
      ...
      prisma.customer.count({
        where: { merchantId, lastSeenAt: { lt: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000) } },
      }),
      // New: joined last 30 days
      prisma.customer.count({
        where: { merchantId, firstSeenAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } },
      }),
    ]);
    ...
    const segments = [
      ...
      { segment: "new",          label: "عملاء جدد",          count: new_customers,  color: "#8B5CF6", description: "انضموا خلال 30 يوماً" },
...
export const getCohorts = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const cohorts: {
      cohort: string; newCustomers: number; avgCLV: number; avgOrders: number; retentionRate: number;
    }[] = [];
    ...
    for (let i = numMonths - 1; i >= 0; i--) {
      ...
      const cohortCustomers = await prisma.customer.aggregate({
        where: { merchantId, firstSeenAt: { gte: cohortStart, lt: cohortEnd } },
        _count: true,
        _avg:   { totalSpent: true, totalOrders: true },
      });

      // Retention: cohort customers who bought again after first month
      const retained = await prisma.customer.count({
        where: {
          merchantId,
          firstSeenAt: { gte: cohortStart, lt: cohortEnd },
          lastSeenAt:  { gte: cohortEnd },
        },
      });

      const total = cohortCustomers._count;
      cohorts.push({
        cohort:        `${cohortStart.getFullYear()}-${String(cohortStart.getMonth() + 1).padStart(2, "0")}`,
        newCustomers:  total,
        avgCLV:        Math.round((cohortCustomers._avg.totalSpent  ?? 0) * 100) / 100,
        avgOrders:     Math.round((cohortCustomers._avg.totalOrders ?? 0) * 10)  / 10,
        retentionRate: total > 0 ? Math.round((retained / total) * 100 * 10) / 10 : 0,
      });
    }
...
export const getTopCustomers = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const customers = await prisma.customer.findMany({
      ...
      select:  {
        customerId: true, name: true, phone: true, email: true, country: true,
        totalSpent: true, totalOrders: true, avgOrderValue: true,
        firstSeenAt: true, lastSeenAt: true, tags: true,
      },
    });

    const withSegment = customers.map(c => ({
      ...c,
      segment: getCustomerSegment(c.totalOrders, c.totalSpent, c.lastSeenAt),
    }));

    res.json({ success: true, data: { customers: withSegment } });
    return;
  } catch (err) {
...
// ─── GET /api/customer-clv/predict/:customerId ──────────────────────────────
export const predictCLV = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // Simple CLV prediction: avg order value × predicted future orders
    const daysSinceFirst = Math.max(1,
      (Date.now() - customer.firstSeenAt.getTime()) / (1000 * 60 * 60 * 24));
    ...
    res.json({
      ...
      data: {
        ...
        predicted6m:    Math.round(predicted6m  * 100) / 100,
        predicted12m:   Math.round(predicted12m * 100) / 100,
        churnRisk,
        daysSinceLast:  Math.round(daysSinceLast),
        ordersPerDay:   Math.round(ordersPerDay * 1000) / 1000,
        segment:        getCustomerSegment(customer.totalOrders, customer.totalSpent, customer.lastSeenAt),
      },
    });
    return;
  } catch (err) {
...
function getCustomerSegment(orders: number, spent: number, lastSeen: Date): string {
  const daysSince = (Date.now() - lastSeen.getTime()) / (1000 * 60 * 60 * 24);
  if (orders >= 5 && spent >= 1000 && daysSince <= 30) return "champion";
