...
// ── Default stages (created on first use) ────────────────────
...
async function ensureStages(merchantId: string) {
...
export async function deleteStage(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (dealsCount > 0) {
      return res.status(400).json({ success: false, message: `لا يمكن حذف مرحلة تحتوي على ${dealsCount} صفقة مفتوحة` })
    }
    const count = await prisma.pipelineStage.count({ where: { merchantId } })
    if (count <= 1) return res.status(400).json({ success: false, message: 'يجب أن يكون هناك مرحلة واحدة على الأقل' })
...
export async function reorderStages(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function createDeal(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (!title || !stageId || value === undefined) {
      return res.status(400).json({ success: false, message: 'title, stageId, value مطلوبة' })
    }

    // تحقق أن الـ stage تابعة للـ merchant
    const stage = await prisma.pipelineStage.findFirst({ where: { id: stageId, merchantId } })
    if (!stage) return res.status(400).json({ success: false, message: 'المرحلة غير موجودة' })

    const count = await prisma.deal.count({ where: { merchantId } })
    const deal  = await prisma.deal.create({
      data: {
        merchantId, dealId: dealId(count + 1), stageId,
...
export async function updateDeal(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (!existing) return res.status(404).json({ success: false, message: 'الصفقة غير موجودة' })

    const { title, value, currency, probability, assignedTo, expectedCloseAt, notes, stageId } = req.body
    const deal = await prisma.deal.update({
      where: { id },
      data: {
        title, value, currency, probability, assignedTo,
        notes, stageId,
...
export async function moveDealStage(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params
    const merchantId = req.merchant!.id
...
export async function addActivity(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params
    const merchantId = req.merchant!.id
...
export async function getReports(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const totalWonValue      = wonDeals.reduce((s, d)  => s + Number(d.value), 0)
    const conversionRate     = deals.length > 0 ? ((wonDeals.length / deals.length) * 100).toFixed(1) : '0'
    const avgDealValue       = deals.length > 0 ? (deals.reduce((s, d) => s + Number(d.value), 0) / deals.length).toFixed(2) : '0'
    ...
    res.json({
      ...
      data: {
        kpis: {
          total: deals.length, open: openDeals.length, won: wonDeals.length, lost: lostDeals.length,
          totalPipelineValue, totalWonValue, conversionRate, avgDealValue,
        },
        funnelData,
