...
export const get3dsDecision = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  // deviceFingerprint received but not used in current logic — prefixed _
  const { bin, amount, currency, isReturningCustomer = false, _deviceFingerprint } = req.body;
  ...
  try {
    ...
    const countryCode = binData.bankCountryCode || null;
    const rule = countryCode ? THREEDS_RULES[countryCode] : null;
    const numAmount = Number(amount);
    let decision = 'FRICTIONLESS'; let reason = ''; let confidence = 0;
    if (rule?.required) { decision='REQUIRED'; reason=rule.reason; confidence=100; }
    else if (rule && numAmount <= rule.exemptBelow) { decision='EXEMPTION'; reason=`${rule.reason} — Amount ${numAmount} ${currency} is below threshold ${rule.exemptBelow}`; confidence=95; }
    else if (isReturningCustomer && numAmount < 500) { decision='FRICTIONLESS'; reason='Returning customer with low-risk amount'; confidence=85; }
    else if (numAmount > 5000) { decision='CHALLENGE'; reason='High amount — 3DS challenge required'; confidence=90; }
    else if (binData.cardType === 'PREPAID') { decision='CHALLENGE'; reason='Prepaid cards have higher fraud risk'; confidence=80; }
    else { decision='FRICTIONLESS'; reason='Standard transaction — frictionless 3DS flow'; confidence=75; }
    const binHistory: any[] = await prisma.$queryRawUnsafe(`SELECT id FROM bin_lookups WHERE "merchantId"=$1 AND bin=$2 ORDER BY "createdAt" DESC LIMIT 10`, merchantId, cleanBin);
    res.json({ success: true, data: { bin: cleanBin, decision, reason, confidence, cardInfo: { brand: binData.cardBrand, type: binData.cardType, country: countryCode, bankName: binData.bankName }, amount: numAmount, currency, rule: rule||null, lookupCount: binHistory.length, recommendation: decision==='EXEMPTION'?'Skip 3DS — apply TRA exemption':decision==='REQUIRED'?'Apply full 3DS challenge':decision==='CHALLENGE'?'Request 3DS challenge for high-risk':'Use frictionless 3DS flow' } }); return;
...
export const getCountry3dsRules = async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const rules = Object.entries(THREEDS_RULES).map(([country, rule]) => ({ country, required: rule.required, exemptBelow: rule.exemptBelow, reason: rule.reason, status: rule.required?'🔴 إجباري':rule.exemptBelow>0?`🟡 اختياري (إعفاء < ${rule.exemptBelow})`:'🟢 اختياري' }));
