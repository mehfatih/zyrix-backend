  ...
  try {
    ...
    const [todayTx, lastHourTx, last5MinTx, pendingTx, todayRevenue, successCount, failedCount] =
      await Promise.all([
        ...
        prisma.transaction.aggregate({ where: { merchantId, status: "SUCCESS", createdAt: { gte: startOfDay } }, _sum: { amount: true } }),
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const [cnt, rev]  = await Promise.all([
        prisma.transaction.count({ where: { merchantId, createdAt: { gte: bucketStart, lt: bucketEnd } } }),
        prisma.transaction.aggregate({ where: { merchantId, status: "SUCCESS", createdAt: { gte: bucketStart, lt: bucketEnd } }, _sum: { amount: true } }),
      ]);
      sparkline.push({ time: bucketEnd.toISOString(), count: cnt, revenue: Number(rev._sum.amount ?? 0) });
    }

    const topCountriesRaw = await prisma.$queryRawUnsafe<Array<{ country: string; cnt: bigint; total: string }>>(
    ...
    res.json({ success: true, data: { overview: { todayTransactions: todayTx, todayRevenue: Number(todayRevenue._sum.amount ?? 0), successRate, pendingCount: pendingTx, tpmLastHour, tpm5Min, lastHourCount: lastHourTx, last5MinCount: last5MinTx }, sparkline, topCountries, alerts, generatedAt: now.toISOString() } });
    return;
  } catch (err) {
...
export const getDrillDown = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<Array<{ dim_val: string; cnt: bigint; total: string; success_cnt: bigint }>>(
      `SELECT "${dim}" as dim_val, COUNT(*) as cnt, SUM(amount)::text as total,
              COUNT(*) FILTER (WHERE status = 'SUCCESS') as success_cnt
       FROM transactions
       WHERE "merchantId" = $1 AND "createdAt" >= $2
       GROUP BY "${dim}" ORDER BY cnt DESC LIMIT 10`,
      merchantId, since
    );
    ...
    const breakdown = rows.map(r => ({
      ...
      count:       Number(r.cnt),
      revenue:     Number(r.total ?? 0),
      successRate: Number(r.cnt) > 0 ? Math.round((Number(r.success_cnt) / Number(r.cnt)) * 100 * 10) / 10 : 0,
    }));
...
export const recordMetric = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const merchantId = req.merchant!.id;
    const { metric, value, currency = "SAR", metadata } = req.body as { metric: string; value: number; currency?: string; metadata?: Record<string, unknown> };
    if (!metric || value === undefined) { res.status(400).json({ success: false, error: "metric and value required" }); return; }
    await prisma.$executeRawUnsafe(
      `INSERT INTO realtime_metrics (id, "merchantId", metric, value, currency, metadata) VALUES ($1, $2, $3, $4, $5, $6)`,
      crypto.randomUUID(), merchantId, metric, value, currency, metadata ? JSON.stringify(metadata) : null
    );
...
export const getSummary = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const [todayAgg, yesterdayAgg, todayCount, yesterdayCount] = await Promise.all([
      prisma.transaction.aggregate({ where: { merchantId, status: "SUCCESS", createdAt: { gte: startOfDay } }, _sum: { amount: true }, _count: true }),
      prisma.transaction.aggregate({ where: { merchantId, status: "SUCCESS", createdAt: { gte: yesterday, lt: startOfDay } }, _sum: { amount: true }, _count: true }),
      prisma.transaction.count({ where: { merchantId, createdAt: { gte: startOfDay } } }),
    ...
    const revChange    = yesterdayRev > 0 ? Math.round(((todayRev - yesterdayRev) / yesterdayRev) * 100 * 10) / 10 : 0;
    const cntChange    = yesterdayCount > 0 ? Math.round(((todayCount - yesterdayCount) / yesterdayCount) * 100 * 10) / 10 : 0;

    res.json({ success: true, data: { today: { revenue: todayRev, transactions: todayCount }, yesterday: { revenue: yesterdayRev, transactions: yesterdayCount }, changes: { revenue: revChange, transactions: cntChange } } });
    return;
  } catch (err) {
