  ...
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      trend.push({
        ...
        rate: t > 0 ? Math.round((s / t) * 100 * 100) / 100 : 0,
        total: t,
      });
    }
    ...
    res.json({
      success: true,
      data: { overall: { total, success, failed, pending, rate }, trend },
    });
    return;
  } catch (err) {
...
export const getByBank = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // Join with bin_lookups to get bank names
    const rows = await prisma.$queryRawUnsafe
      { bank_name: string; total: bigint; success_cnt: bigint; failed_cnt: bigint }[]
    >(`
      SELECT
        COALESCE(bl."bankName", 'Unknown') as bank_name,
        COUNT(t.id) as total,
      ...
      GROUP BY COALESCE(bl."bankName", 'Unknown')
      ORDER BY total DESC
      LIMIT 15
    `, merchantId, since);
    ...
    const banks = rows.map(r => ({
      ...
      success:     Number(r.success_cnt),
      failed:      Number(r.failed_cnt),
      successRate: Number(r.total) > 0
        ? Math.round((Number(r.success_cnt) / Number(r.total)) * 100 * 10) / 10
        : 0,
    }));
...
// ─── GET /api/success-rate/by-country ───────────────────────────────────────
export const getByCountry = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe
      { country: string; total: bigint; success_cnt: bigint }[]
    >(`
      SELECT country,
        COUNT(*) as total,
...
// ─── GET /api/success-rate/by-method ────────────────────────────────────────
export const getByMethod = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe
      { method: string; total: bigint; success_cnt: bigint; avg_amount: string }[]
    >(`
      SELECT method,
        COUNT(*) as total,
        COUNT(*) FILTER (WHERE status = 'SUCCESS') as success_cnt,
...
// ─── GET /api/success-rate/failure-reasons ──────────────────────────────────
export const getFailureReasons = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const reasons = rows.map(r => ({
      code:  r.error_code,
      count: Number(r.cnt),
      label: getErrorLabel(r.error_code),
    }));
